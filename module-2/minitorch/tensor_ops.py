...
def tensor_map(fn: Callable[[float], float]) -> Any:
    ...
    * Fill in the `out` array by applying `fn` to each
    ...
    def _map(
        ...
        out_index = np.ndarray(out_shape.size, dtype=int)
        in_index = np.ndarray(in_shape.size, dtype=int)

        for ordinal in range(out.size):
            to_index(ordinal, out_shape, out_index)
            broadcast_index(out_index, out_shape, in_shape, in_index)
            out_storage_index = index_to_position(out_index, out_strides)
            in_storage_index = index_to_position(in_index, in_strides)
            out[out_storage_index] = fn(in_storage[in_storage_index])
...
def tensor_zip(fn: Callable[[float, float], float]) -> Any:
    ...
    * Fill in the `out` array by applying `fn` to each
    ...
    def _zip(
        ...
        out_index = np.ndarray(out_shape.size, dtype=int)
        a_index = np.ndarray(a_shape.size, dtype=int)
        b_index = np.ndarray(b_shape.size, dtype=int)
        ...
        for ordinal in range(out.size):
            ...
            out_storage_index = index_to_position(out_index, out_strides)
            a_storage_index = index_to_position(a_index, a_strides)
            b_storage_index = index_to_position(b_index, b_strides)
            out[out_storage_index] = fn(a_storage[a_storage_index], b_storage[b_storage_index])
...
def tensor_reduce(fn: Callable[[float, float], float]) -> Any:
    ...
    * `out_shape` will be the same as `a_shape`
    ...
    def _reduce(
        ...
        out_index = np.ndarray(len(out_shape), dtype=int)
        a_index = np.ndarray(len(a_shape), dtype=int)
        ...
        for ordinal in range(out.size):
            ...
            # Get the output storage position
            ...
            for reduce_idx in range(a_shape[reduce_dim]):
                ...
                # Apply reduction function (accumulate)
                # Note: out is pre-initialized with start value
