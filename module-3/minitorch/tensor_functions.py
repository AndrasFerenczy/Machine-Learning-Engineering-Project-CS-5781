...
class LT(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, Tensor]:
        (a, b) = ctx.saved_values
        a_zero_tensor = zeros(a.shape, backend=a.backend)
        b_zero_tensor = zeros(a.shape, backend=a.backend)
        return a_zero_tensor, b_zero_tensor
...
class Permute(Function):
    ...
    def forward(ctx: Context, a: Tensor, order: Tensor) -> Tensor:
        ...
        ctx.save_for_backward(tuple(order_list))
        new_tensor_data = a._tensor.permute(*order_list)

        return minitorch.Tensor(new_tensor_data, backend=a.backend)

    @staticmethod
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        (order_list,) = ctx.saved_values
        inverse = np.zeros(len(order_list))
        for i, o in enumerate(order_list):
            inverse[o] = i
        grad_input = minitorch.Tensor(grad_output._tensor.permute(*inverse), backend=grad_output.backend)
