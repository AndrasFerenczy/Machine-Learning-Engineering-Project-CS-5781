...
def tensor_data(
    ...
    if shape is None:
        shape = draw(shapes())  # pyright: ignore[reportCallIssue]
    size = int(minitorch.prod(shape))  # pyright: ignore[reportArgumentType]
    data = draw(lists(numbers, min_size=size, max_size=size))
    permute: List[int] = draw(permutations(range(len(shape))))  # pyright: ignore[reportArgumentType]
    permute_shape = tuple([shape[i] for i in permute])  # pyright: ignore[reportOptionalSubscript]
    ...
    assert ret.shape[0] == shape[0]  # pyright: ignore[reportOptionalSubscript]
    return ret
